...
- (void)viewDidLoad
{
    ...
    _textNode.transform = CATransform3DRotate(_textNode.transform, 45, 0, 0, 1);
}

- (void)viewWillLayoutSubviews
{
    // place the text node in the top half of the screen, with a bit of padding
    _textNode.frame = CGRectMake(0, 70, self.view.bounds.size.width,20 + _textNode.calculatedSize.height);
}
...
- (void)editableTextNodeDidUpdateText:(ASEditableTextNode *)editableTextNode {
    ...
    NSLog(@"%@", editableTextNode.attributedText.string);
    
    [editableTextNode measure:CGSizeMake(320, FLT_MAX)];
    editableTextNode.frame = CGRectMake(0, 70, 320, editableTextNode.calculatedSize.height);
    editableTextNode.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.1f];
    NSLog(@"%@", NSStringFromCGSize(editableTextNode.frame.size));
